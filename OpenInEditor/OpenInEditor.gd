...
static var POPUP_MENU_ID = 5164
static var NODE_PATH_CONTEXT_MENU = "@PopupMenu@5811"
...
func popup_menu_about_to_pop_up() -> void:
	...
	file_system_dock_context_menu\
	.add_icon_item(open_in_icon, "Open in external editor", POPUP_MENU_ID)
...
func popup_menu_id_pressed(id: int) -> void:
	...
	var external_editor_path := EditorInterface\
	.get_editor_settings()\
	.get_setting("text_editor/external/exec_path")
	if external_editor_path == null:
		print("[OpenExternally] Please ensure setting text_editor/external/exec_path\
		 is set to the path of your editor in Editor settings.")
	
	var location := ProjectSettings.globalize_path(Array(EditorInterface.get_selected_paths()).front())
	OS.execute(external_editor_path, PackedStringArray([location]))
